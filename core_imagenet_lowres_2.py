...
def get_noise_batch(batch,index_tensor,this_batch_size,sigma):
    ...
    batch = batch.repeat((this_batch_size, 1, 1, 1)).cuda()
    noise = torch.randn_like(batch, device='cuda') * sigma
    noised_batch=batch + noise
    noised_batch = F.interpolate(noised_batch,size=(224,224), mode='bilinear')
...
class Smooth(object):
    ...
    def __init__(self, base_classifier_l: torch.nn.Module,base_classifier_r: torch.nn.Module, num_classes: int, sigma: float):
        ...
        """
        self.base_classifier_l1 = base_classifier_l1
        self.base_classifier_l2 = base_classifier_l2
        self.base_classifier_r1 = base_classifier_r1
        self.base_classifier_r2 = base_classifier_r2
    ...
    def certify(self, x: torch.tensor, n0: int, n: int, alpha: float, batch_size: int) -> (int, float):
        ...
        """
        self.base_classifier_l1.eval()
        self.base_classifier_l2.eval()
        self.base_classifier_r1.eval()
        self.base_classifier_r2.eval()
       ...
       # radius=radius0+radius1+radius2+radius3
        radius_org=(radius0**2+radius1**2)**0.5
    ...
    def _sample_noise(self, x: torch.tensor, num: int, batch_size) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            for _ in range(ceil(num / batch_size)):
                ...
                predictions0 = (self.base_classifier_l1(noised_batch_l)+self.base_classifier_l2(noised_batch_l)).argmax(1
                noised_batch_r=get_noise_batch(batch,index_tensor_r,this_batch_size,self.sigma)
                predictions1 = (self.base_classifier_r(noised_batch_r1)+self.base_classifier_r(noised_batch_r2)).argmax(1)
    ...
    def _sample_noise_smoothing(self, x: torch.tensor, num: int, batch_size) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            for _ in range(ceil(num / batch_size)):
                ...
                predictions=F.softmax(predictions*128,dim=1)
